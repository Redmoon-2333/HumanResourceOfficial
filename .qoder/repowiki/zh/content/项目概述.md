...
- [RagController.java](file://src\main\java\com\redmoon2333\controller\RagController.java) - *新增RAG功能*
- [AIChatController.java](file://src\main\java\com\redmoon2333\controller\AIChatController.java) - *新增AI对话功能*
- [RagConfig.java](file://src\main\java\com\redmoon2333\config\RagConfig.java) - *新增RAG配置*
- [QdrantConfig.java](file://src\main\java\com\redmoon2333\config\QdrantConfig.java) - *新增向量数据库配置*
- [SaaLLMConfig.java](file://src\main\java\com\redmoon2333\config\SaaLLMConfig.java) - *新增大模型配置*
- [ToolService.java](file://src\main\java\com\redmoon2333\service\ToolService.java) - *新增工具调用服务*
- [RagInitRequest.java](file://src\main\java\com\redmoon2333\dto\RagInitRequest.java) - *新增RAG初始化请求*
- [RagChatRequest.java](file://src\main\java\com\redmoon2333\dto\RagChatRequest.java) - *新增RAG聊天请求*
- [00-使用说明.txt](file://src\main\resources\rag-knowledge-base\00-使用说明.txt) - *新增知识库使用说明*
</cite>

## 更新摘要
**变更内容**   
- 在项目简介中新增了AI RAG和Tool Calling功能的描述
- 新增了AI智能功能模块章节
- 更新了系统架构图以包含AI服务组件
- 新增了RAG知识库管理流程图
- 新增了AI对话与工具调用序列图
...
- **分页查询支持**：高效的数据分页处理
- **AI智能增强**：集成RAG（检索增强生成）和Tool Calling技术，提供智能化服务
...
- [SaaLLMConfig.java](file://src\main\java\com\redmoon2333\config\SaaLLMConfig.java#L1-L85) - *新增大模型配置*
...
- [AIChatController.java](file://src\main\java\com\redmoon2333\controller\AIChatController.java#L1-L277) - *新增AI控制器*
- [RagController.java](file://src\main\java\com\redmoon2333\controller\RagController.java#L1-L106) - *新增RAG控制器*
...
系统集成了RAG（检索增强生成）技术，通过向量数据库存储和检索知识库信息，为AI对话提供上下文支持。

```mermaid
flowchart TD
Start([RAG知识库初始化]) --> ValidateAuth{验证用户权限}
ValidateAuth --> |无权限| AccessDenied[权限拒绝]
ValidateAuth --> |有权限| CheckForceReindex{检查是否强制重建}
CheckForceReindex --> |是| DeleteCollection[删除现有Collection]
CheckForceReindex --> |否| EnsureCollection[确保Collection存在]
DeleteCollection --> EnsureCollection
EnsureCollection --> ScanFiles[扫描知识库文件]
ScanFiles --> BuildMD5Cache[构建MD5缓存]
BuildMD5Cache --> ProcessFiles[处理每个文件]
ProcessFiles --> ParseDocument[解析文档内容]
ParseDocument --> SplitText[文本分块]
SplitText --> AddToVectorStore[添加到向量存储]
AddToVectorStore --> UpdateCache[更新MD5缓存]
UpdateCache --> NextFile{还有文件?}
NextFile --> |是| ProcessFiles
NextFile --> |否| GenerateReport[生成初始化报告]
GenerateReport --> End([返回结果])
AccessDenied --> End
```

**图表来源**
- [RagController.java](file://src\main\java\com\redmoon2333\controller\RagController.java#L21-L106)
- [RagManagementService.java](file://src\main\java\com\redmoon2333\service\RagManagementService.java#L32-L317) - *新增RAG管理服务*
- [RagConfig.java](file://src\main\java\com\redmoon2333\config\RagConfig.java#L1-L63) - *新增RAG配置*

### AI对话与工具调用

系统实现了AI对话功能，支持RAG增强和工具调用，为用户提供智能化服务。

```mermaid
sequenceDiagram
participant Client as 客户端
participant AIChatController as AI对话控制器
participant AIChatService as AI服务
participant RagRetrievalService as RAG检索服务
participant ToolService as 工具服务
participant Qwen as 通义千问大模型
Client->>AIChatController : POST /api/ai/chat-with-rag
AIChatController->>AIChatService : chatWithRag(userId, message, useRAG, enableTools)
AIChatService->>AIChatService : 构建系统提示
alt 启用RAG
AIChatService->>RagRetrievalService : retrieveRelevantDocuments(message)
RagRetrievalService-->>AIChatService : 相关文档
AIChatService->>AIChatService : 将文档添加到上下文
end
alt 启用工具调用
AIChatService->>ToolService : getAvailableTools()
ToolService-->>AIChatService : 工具列表
AIChatService->>AIChatService : 配置工具调用
end
AIChatService->>Qwen : 发送消息(含上下文和工具配置)
Qwen-->>AIChatService : 流式返回响应
AIChatService-->>AIChatController : 流式响应
AIChatController-->>Client : 流式输出
```

**图表来源**
- [AIChatController.java](file://src\main\java\com\redmoon2333\controller\AIChatController.java#L23-L277)
- [AIChatService.java](file://src\main\java\com\redmoon2333\service\AIChatService.java) - *新增AI服务*
- [ToolService.java](file://src\main\java\com\redmoon2333\service\ToolService.java#L18-L95) - *新增工具服务*
- [SaaLLMConfig.java](file://src\main\java\com\redmoon2333\config\SaaLLMConfig.java#L1-L85) - *新增大模型配置*

### RAG知识库配置

系统通过配置文件定义RAG相关参数，支持灵活调整知识库处理策略。

```yaml
# RAG功能配置
rag:
  # 知识库文件目录路径
  knowledgeBasePath: src/main/resources/rag-knowledge-base
...
**章节来源**
- [RagConfig.java](file://src\main\java\com\redmoon2333\config\RagConfig.java#L1-L63)
- [00-使用说明.txt](file://src\main\resources\rag-knowledge-base\00-使用说明.txt#L1-L26) - *新增知识库使用说明*
...
系统集成了先进的AI技术，提供智能化服务：

- **Spring AI**：Spring官方AI框架，提供统一的AI服务接口
- **通义千问大模型**：阿里巴巴研发的大型语言模型，提供强大的自然语言处理能力
- **Qdrant向量数据库**：高性能向量数据库，支持相似性搜索
- **RAG技术**：检索增强生成，结合知识库提供准确的AI回复
- **Tool Calling**：工具调用，允许AI调用系统功能获取实时数据
...
- [application.yml](file://src\main\resources\application.yml#L1-L62)
- [QdrantConfig.java](file://src\main\java\com\redmoon2333\config\QdrantConfig.java#L1-L74) - *新增向量数据库配置*
...
6. **智能增强**：集成RAG和Tool Calling技术，提供智能化服务
